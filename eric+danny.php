<!DOCTYPE html>
<html>
<head>
	<meta charset="utf-8">
	<title>Eric, Danny and Josh | Odd Student Out | Assignment 5</title>
	<link rel="stylesheet" href="css/styles-two.css">
	<link rel="stylesheet" href="css/navigation.css">
	<link href="https://fonts.googleapis.com/css?family=Alegreya|Roboto|Roboto+Condensed" rel="stylesheet">
</head>
<body class="grid-container">
	<div class="head">
		<header>
			<a href="index.php">
				<h1>Team Seoul - Odd Student Out</h1>
			</a>
		</header>
	</div>
	<div class="nav-bar">
		<?php include "inc/nav.inc"; ?>
	</div>

	<aside class="info-one">
		<h2>Eric Feirouz and Danny Aronson</h2>
		<figure class="pic-one">
			<img src="images/eric+danny.jpeg" alt="Eric and Danny">
		</figure>
		<ul>
			<li> Black Hair </li>
			<li> Small Muscles</li>
			<li> Class of 2019 </li>
			<li> Mechanical Engineering Majors</li>
			<li> Did Not Grow Up In New York</li>
			<li> Played Baseball and Swam in High School </li>
			<li> Lived in Susan B Anthony </li>
			<li> Loves Dinosaur Barbeque</li>
			<li> Breaststrokers and Butterflyers</li>
			<li> Practices Swimming from 2:00 to 4:00</li>
			<li> Lifts from 9:00 to 10:00</li>
			<li> Tutors </li>
			<li> Beginning Full Time Jobs </li>
		</ul>
	</aside>

	<aside class="info-two">
		<h2>Josh Dennis</h2>
		<figure>
			<img src="images/josh.jpg" alt="Josh">
		</figure>
		<ul>
			<li> Red Hair </li>
			<li> Big Muscles </li>
			<li> Class of 2020 </li>
			<li> Mathematics Major </li>
			<li> Grew Up in New York</li>
			<li> Played Tennis and Swam in High School </li>
			<li> Lived in the Quad </li>
			<li> Loves Moes Southwest Grille</li>
			<li> Sprint Freestyler</li>
			<li> Practices Swimming from 5:00 to 7:00</li>
			<li> Lifts from 11:00 to 12:00</li>
			<li> Lifeguard </li>
			<li> Beginning an Internship</li>
		</ul>
	</aside>
	
	<main class="more-info">
		<div class="large-container">
			<h3>Background</h3>
			<h4>Eric and Danny</h4>
			<figure>
				<img src="images/dino-bbq.png" alt="Dinosaur BBQ">
			</figure>
			<p>Eric and Danny are currently seniors at the University of Rochester. Before college, Eric and Danny both grew up outside of New York State. They both grew up playing baseball and swimming in high school. Eric and Danny have both been competitive swimmers since they were 8 years old. During the off-season they both like to golf and eat at Dinosaur Barbeque, our favorite restaurant. We have been Mechanical Engineering Majors since our Freshman Year, despite disliking mathematics. We are currently taking a robotics class together. Eric and Danny both lived in Susan B Anthony their Freshman year.</p> 
			<h4 class="schway">Josh</h4>
			<p class="schway">Josh is a junior at the University of Rochester. Before College, Josh grew up in New York State. Josh grew up playing tennis and swimming in high school. Josh has been a competitve swimmers since he was 11 years old. During the off-season Josh likes to play video games and eat at Moe's Southwest Grille, his favorite restaurant. Josh decided upon his mathematics major after his Freshman Year. He loves math and hope to continue to build on the current coursework he has taken. He is currently taking a class that involves heavy applications of linear algebra. Josh lived in the quad his freshman year.</p>
			<h3>In the Pool</h3>
			<div class="flex-container">
				<div class="first">
					<h4>Eric and Danny</h4>
					<p>In the pool, Eric and Danny are both breaststrokers and love to train breaststroke during practice. They practice during the day from 2:00 to 4:00. They lift during the day from 9:00 to 10:00. When they are not training breaststroke, they like to mix in butterfly. Danny and Eric will both be attending the 2019 University of Rochester Alumni meet, representing the Alumni Team. They will be going head to head with Team Blue, the current swimmers at the Univerity of Rochester. Eric and Danny will be swimming the 100 Fly and 100 Breaststroke at this meet.</p>
				</div>
				<div class="second">
					<h4 class="schway">Josh</h4>
					<p class="schway">In the pool, Josh is a sprint freestyler and hates training breaststroke during practice. Josh practices during the day from 5:00 to 7:00. He lifts during the day from 11:00 to 12:00. When he is not training freestyle, Josh likes to mix in backstroke. Josh will be attending the 2019 University of Rochester Alumni Meet, but will be representing Team Blue. He will go head to head against Eric and Danny and the Alumni Team. Josh will be swimming the 50 free and 100 free at this meet.</p>
				</div>
			</div>
			<h3>Jobs</h3>
			<h4>Eric and Danny</h4>
			<p>Eric and Danny both work as tutors on Campus. Eric and Danny like to tutor is subjects such as Thermodynamics, Statics, Physics, Java and Economics. Eric and Danny were unable to get jobs as lifeguards because they were not lifeguard certified. They were not excited by the idea of lifeguarding as it sounded boring to them. After this year, Danny and Eric will be beginning their full time jobs.</p>
			<h4 class="schway">Josh</h4>
			<figure class="special">
				<img src="images/red-cross.png" alt="American Red Cross Logo">
			</figure>
			<p class="schway">Josh works as a lifeguard on campus. Josh began lifeguarding as a senior in highschool and has been certified ever since. Josh loves his job as a lifeguard because he knows the importance of having a professional keeping a pool area safe. Josh loves his job and takes it very seriously. After this year, Josh will be beginning an internship.</p>
		</div>
	</main>
	

	<footer class="foot">CSC 174: Advanced Front-end Web Design and Development</footer>

	<script src="//code.jquery.com/jquery.js"></script>
	<script src="js/menu-highlighter.js"></script>
</body>
</html>
